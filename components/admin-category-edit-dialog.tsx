  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-6">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby={`edit-category-${category.id}`} className="relative w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-7">
        ...
        <form action={updateCommonCategory} className="space-y-5">
          ...
          <div className="flex gap-3 pt-1"><Button type="button" variant="outline" onClick={() => setOpen(false)} className="h-11 flex-1 rounded-xl">取消</Button><Button type="submit" className="h-11 flex-[1.5] rounded-xl bg-emerald-600 hover:bg-emerald-700">保存修改</Button></div>
        </form>
      </div>
    </div> : null}
  </>;
