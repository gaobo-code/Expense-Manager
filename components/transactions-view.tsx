...
export function TransactionsView({ transactions, categories, customers, hasError, errorCode }: { transactions: Transaction[]; categories: Category[]; customers: Customer[]; hasError: boolean; errorCode?: string }) {
  ...
  return <PageShell><section>
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="w-full overflow-hidden"><table className="w-full table-fixed border-collapse text-center"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500 md:text-xs md:tracking-wider"><tr><th className="w-[28%] px-2 py-3 md:w-auto md:px-5 md:py-4">{t("date")}</th><th className="w-[28%] px-2 py-3 md:w-auto md:px-5 md:py-4">{t("category")}</th><th className="hidden px-5 py-4 md:table-cell">{labels.account}</th><th className="hidden px-5 py-4 md:table-cell">{labels.customer}</th><th className="w-[32%] px-2 py-3 md:w-auto md:px-5 md:py-4">{t("amount")}</th><th className="w-[12%] md:w-16" /></tr></thead><tbody className="divide-y divide-slate-100">
        {transactions.map((transaction) => { const Icon = icons[transaction.account_type] ?? WalletCards; const category = categories.find((item) => item.id === transaction.category_id); return <tr key={transaction.id} className="hover:bg-slate-50"><td className="break-words px-2 py-4 text-xs font-medium text-slate-700 md:px-5 md:text-sm">{formatDate(`${transaction.transaction_date}T00:00:00Z`)}</td><td className="break-words px-2 py-4 md:px-5"><span className="inline-flex max-w-full break-words rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 md:px-3 md:text-sm">{category ? getCategoryName(category, language) : transaction.category}</span></td><td className="hidden px-5 py-4 text-sm text-slate-600 md:table-cell"><span className="inline-flex items-center gap-2"><Icon size={16}/>{labels[transaction.account_type]}</span></td><td className="hidden px-5 py-4 text-sm text-slate-600 md:table-cell">{transaction.customers?.name ?? "—"}</td><td className="break-words px-2 py-4 text-xs font-semibold tabular-nums md:px-5 md:text-sm">{new Intl.NumberFormat(zh ? "zh-CN" : "en-US", { style: "currency", currency: transaction.currency }).format(transaction.amount)}</td><td className="pr-1 md:pr-3"><TransactionDialog transaction={transaction} categories={categories} customers={customers} /></td></tr>; })}
      </tbody></table></div>}
    </div>
  </section></PageShell>;
}
