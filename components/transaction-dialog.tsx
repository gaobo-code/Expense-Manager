...
function QuickCreateDialog({ kind, labels, roots, language, onClose, onCreated }: { kind: QuickKind; labels: Record<string, string>; roots: Category[]; language: "zh" | "en"; onClose: () => void; onCreated: (item: { id: number; name: string; nameZh?: string; nameEn?: string; parentId?: number | null }) => void }) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-end justify-center text-left font-sans text-slate-950 sm:items-center sm:p-6"><button type="button" aria-label={labels.close} onClick={onClose} className="absolute inset-0 bg-slate-950/55"/><div role="dialog" aria-modal="true" className="relative max-h-[calc(100%-1rem)] w-full overflow-y-auto overscroll-y-contain rounded-t-3xl bg-white p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl sm:max-h-full sm:max-w-lg sm:rounded-2xl sm:p-7"><div className="mb-6 flex items-start justify-between gap-4"><div><h3 className="text-xl font-bold">{isCategory ? labels.addCategory : labels.addCustomer}</h3><p className="mt-1 text-sm text-slate-500">{labels.quickHint}</p></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-full bg-slate-100"><X size={18}/></button></div>{isCategory ? <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><Field label={labels.chineseName}><Input autoFocus value={nameZh} onChange={(event) => { setNameZh(event.target.value); setError(false); }} maxLength={60} className="h-11 scroll-my-20 rounded-xl"/></Field><Field label={labels.englishName}><Input value={nameEn} onChange={(event) => { setNameEn(event.target.value); setError(false); }} maxLength={60} className="h-11 scroll-my-20 rounded-xl"/></Field></div><Field label={labels.parentCategory}><select value={parentId} onChange={(event) => setParentId(event.target.value)} className={selectClass}><option value="">{labels.noParent}</option>{roots.map((root) => <option key={root.id} value={root.id}>{getCategoryName(root, language)}</option>)}</select></Field>{error ? <p className="text-sm text-red-600">{!nameZh.trim() && !nameEn.trim() ? labels.categoryRequired : labels.failed}</p> : null}</div> : <Field label={`${labels.customerName} *`}><Input autoFocus value={name} onChange={(event) => { setName(event.target.value); setError(false); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); submit(); } }} maxLength={100} className="h-11 scroll-my-20 rounded-xl"/>{error ? <p className="mt-2 text-sm text-red-600">{labels.failed}</p> : null}</Field>}<div className="mt-6 flex gap-3"><Button type="button" variant="outline" disabled={pending} onClick={onClose} className="h-11 flex-1 rounded-xl">{labels.cancel}</Button><Button type="button" disabled={pending} onClick={submit} className="h-11 flex-[1.5] rounded-xl bg-emerald-600 hover:bg-emerald-700">{pending ? labels.adding : labels.confirm}</Button></div></div></div>;
}
